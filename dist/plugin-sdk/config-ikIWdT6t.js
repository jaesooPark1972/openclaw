import { G as CHANNEL_IDS, I as success, M as info, S as resolveConfigDir, V as getChildLogger, X as normalizeChatChannelId, b as jidToE164, u as defaultRuntime, w as resolveUserPath } from "./exec-Cm9b2r9Q.js";
import { A as normalizeAccountId, T as DEFAULT_AGENT_ID, c as resolveDefaultAgentId, j as normalizeAgentId, s as resolveAgentWorkspaceDir, w as DEFAULT_ACCOUNT_ID } from "./agent-scope-DdwUKIOe.js";
import { a as resolveOAuthDir, n as resolveConfigPath, r as resolveDefaultConfigCandidates, s as resolveStateDir } from "./paths-BYpoyRv5.js";
import { B as shouldDeferShellEnvFallback, P as DEFAULT_CONTEXT_TOKENS, R as loadShellEnvFallback, V as shouldEnableShellEnvFallback, o as parseModelRef, z as resolveShellEnvFallbackTimeoutMs } from "./model-selection-C_iUlGaw.js";
...
const DmConfigSchema = z.object({ historyLimit: z.number().int().min(0).optional() });
...
const MarkdownConfigSchema = z.object({ tables: MarkdownTableModeSchema.optional() }).optional();
...
const AgentToolsSchema = z.object({
	...
	sandbox: z.object({ tools: ToolPolicySchema }).optional()
}).superRefine((value, ctx) => {
	if (value.allow && value.allow.length > 0 && value.alsoAllow && value.alsoAllow.length > 0) ctx.addIssue({
		code: z.ZodIssueCode.custom,
...
const MemorySearchSchema = z.object({
	...
	experimental: z.object({ sessionMemory: z.boolean().optional() }).optional(),
	provider: z.union([
		z.literal("openai"),
		z.literal("local"),
		z.literal("gemini")
...
const ToolsSchema = z.object({
	...
	message: z.object({
		...
		broadcast: z.object({ enabled: z.boolean().optional() }).optional()
	}).optional(),
	...
	subagents: z.object({ tools: ToolPolicySchema }).optional(),
	sandbox: z.object({ tools: ToolPolicySchema }).optional()
}).superRefine((value, ctx) => {
	if (value.allow && value.allow.length > 0 && value.alsoAllow && value.alsoAllow.length > 0) ctx.addIssue({
		code: z.ZodIssueCode.custom,
...
const TelegramCapabilitiesSchema = z.union([z.array(z.string()), z.object({ inlineButtons: TelegramInlineButtonsScopeSchema.optional() })]);
...
const TelegramGroupSchema = z.object({
	...
	allowFrom: z.array(z.union([z.string(), z.number()])).optional(),
	systemPrompt: z.string().optional(),
	topics: z.record(z.string(), TelegramTopicSchema).optional()
});
...
const TelegramAccountSchemaBase = z.object({
	...
	network: z.object({ autoSelectFamily: z.boolean().optional() }).optional(),
	proxy: z.string().optional(),
...
const TelegramConfigSchema = TelegramAccountSchemaBase.extend({ accounts: z.record(z.string(), TelegramAccountSchema).optional() }).superRefine((value, ctx) => {
	requireOpenAllowFrom({
		policy: value.dmPolicy,
		allowFrom: value.allowFrom,
		ctx,
...
const DiscordConfigSchema = DiscordAccountSchema.extend({ accounts: z.record(z.string(), DiscordAccountSchema).optional() });
...
const GoogleChatAccountSchema = z.object({
	...
	groupPolicy: GroupPolicySchema.optional().default("allowlist"),
	groupAllowFrom: z.array(z.union([z.string(), z.number()])).optional(),
	groups: z.record(z.string(), GoogleChatGroupSchema).optional(),
...
const GoogleChatConfigSchema = GoogleChatAccountSchema.extend({
	accounts: z.record(z.string(), GoogleChatAccountSchema).optional(),
...
const SlackAccountSchema = z.object({
	...
	channels: z.record(z.string(), SlackChannelSchema).optional(),
	heartbeat: ChannelHeartbeatVisibilitySchema,
	responsePrefix: z.string().optional()
});
...
const SignalConfigSchema = SignalAccountSchemaBase.extend({ accounts: z.record(z.string(), SignalAccountSchema).optional() }).superRefine((value, ctx) => {
	requireOpenAllowFrom({
		policy: value.dmPolicy,
		allowFrom: value.allowFrom,
		ctx,
...
const IMessageConfigSchema = IMessageAccountSchemaBase.extend({ accounts: z.record(z.string(), IMessageAccountSchema).optional() }).superRefine((value, ctx) => {
	requireOpenAllowFrom({
		policy: value.dmPolicy,
		allowFrom: value.allowFrom,
		ctx,
...
const BlueBubblesAccountSchemaBase = z.object({
	...
	blockStreaming: z.boolean().optional(),
	blockStreamingCoalesce: BlockStreamingCoalesceSchema.optional(),
	groups: z.record(z.string(), BlueBubblesGroupConfigSchema).optional(),
	heartbeat: ChannelHeartbeatVisibilitySchema,
	responsePrefix: z.string().optional()
});
...
const BlueBubblesConfigSchema = BlueBubblesAccountSchemaBase.extend({
	accounts: z.record(z.string(), BlueBubblesAccountSchema).optional(),
...
const MSTeamsTeamSchema = z.object({
	...
	toolsBySender: ToolPolicyBySenderSchema$1,
	replyStyle: MSTeamsReplyStyleSchema.optional(),
	channels: z.record(z.string(), MSTeamsChannelSchema).optional()
});
const MSTeamsConfigSchema = z.object({
	...
	dms: z.record(z.string(), DmConfigSchema).optional(),
	replyStyle: MSTeamsReplyStyleSchema.optional(),
	teams: z.record(z.string(), MSTeamsTeamSchema).optional(),
	mediaMaxMb: z.number().positive().optional(),
...
const AudioSchema = z.object({ transcription: TranscribeAudioSchema }).optional();
...
const ApprovalsSchema = z.object({ exec: ExecApprovalForwardingSchema }).optional();
...
const InternalHooksSchema = z.object({
	...
	load: z.object({ extraDirs: z.array(z.string()).optional() }).optional(),
...
const BrowserSnapshotDefaultsSchema = z.object({ mode: z.literal("efficient").optional() }).optional();
...
const OpenClawSchema = z.object({
	...
	browser: z.object({
		...
		}).refine((value) => value.cdpPort || value.cdpUrl, { message: "Profile must set cdpPort or cdpUrl" })).optional()
	}).optional(),
	ui: z.object({
	...
	audio: AudioSchema,
	media: z.object({ preserveFilenames: z.boolean().optional() }).optional(),
	...
	discovery: z.object({
		wideArea: z.object({ enabled: z.boolean().optional() }).optional(),
