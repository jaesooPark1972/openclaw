...
for name, key in keys.items():
    if name == "Gemini": success, msg = test_gemini(key)
    elif name == "OpenAI": success, msg = test_openai(key)
    elif name == "DeepSeek": success, msg = test_deepseek(key)
    elif name == "Groq": success, msg = test_groq(key)
    
    results[name] = {"success": success, "message": msg[:200] if not success else "OK"}
